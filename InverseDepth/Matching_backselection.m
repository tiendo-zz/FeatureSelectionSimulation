function [PairWiseMatches,PairWiseMatches_homo,featureExtracted,match_id,ImageId,t] = Matching_backselection(img_idx,featureExtracted,match_id_candid,intrinsic,measurePosesId,rate,NumOfPoses,t)
...
ImageId = round((measurePosesId(end)+measurePosesId(end-1))/2);
dlmwrite([output_folder,'/match_id_',num2str(img_idx),'.txt'],ImageId);
command = [datapath,'ExtractFREAK.sh ',image_folder,' ',config_folder,' ',output_folder,' ',num2str(rate),' ',num2str(img_idx),' ',num2str(1)];
tic;
system(command);
t(img_idx,1) = tic;
