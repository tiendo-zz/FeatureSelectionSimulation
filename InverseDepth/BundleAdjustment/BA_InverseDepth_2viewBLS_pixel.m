function [FeatureBag, AbsolutePoses, CameraParams,PoseGraphMatrix,normHist, gradnormHist,EraseId] = BA_InverseDepth_2viewBLS_pixel(FeatureBag, AbsolutePoses, PoseGraphMatrix, featureExtracted, usedPoses, CameraParams)
...
for featk = featureAvailableList %0:size(PoseGraphMatrix,1)-1
   ...
   for l = 0:length(poses_see_featk)-1
      ...
      H_pixel = StaticPixelToHomogeneousDerivative(Ck_p_f(1:2) / Ck_p_f(3),CameraParams.fc,CameraParams.cc,CameraParams.kc,CameraModel); 
      Ck_z_f_hat = StaticDistort(CameraParams.fc,CameraParams.cc,CameraParams.kc,Ck_p_f(1:2) / Ck_p_f(3),CameraModel);
      ...
      [Jfc,Jcc,residual_tmp,flag] = dprojection_dfc_dcc(Cr_p_f, Ck_R_W, W_R_Cr,W_p_Cr, W_p_Ck, Ck_z_f,CameraParams,CameraModel);
      if isempty(Jfc)
      ...
      if usedPoses(poses_see_featk(l+1)) == 2
         Jpose(2*meascount+1:2*meascount+2, 6*(poses_see_featk(l+1)-1)+1:6*(poses_see_featk(l+1)-1)+5) = H_pixel*[Jp Jq];
      else
          if usedPoses(poses_see_featk(l+1)) ~= 1      
              Jpose(2*meascount+1:2*meascount+2, 6*(poses_see_featk(l+1)-2)+5+1:6*(poses_see_featk(l+1)-2)+5+6) = H_pixel*[Jp Jq];
          end
      end
      ...
      if ref_idx_usePoses ~= 1      
         if usedPoses(ref_idx_usePoses) == 2
            Jpose(2*meascount+1:2*meascount+2, 6*(ref_idx_usePoses-1)+1:6*(ref_idx_usePoses-1)+5) = H_pixel*[Jpr Jqr];
         else
             if usedPoses(ref_idx_usePoses) ~= 1      
                Jpose(2*meascount+1:2*meascount+2, 6*(ref_idx_usePoses-2)+5+1:6*(ref_idx_usePoses-2)+5+6) = H_pixel*[Jpr Jqr];
             end
         end
      end
      ...
      Jfeat(2*meascount+1:2*meascount+2, 3*featcount+1:3*featcount+3) = H_pixel*[Jtheta Jphi Jro];              
