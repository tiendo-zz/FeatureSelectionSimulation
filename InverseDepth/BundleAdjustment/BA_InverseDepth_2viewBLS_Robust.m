function [FeatureBag, AbsolutePoses, normHist, gradnormHist,EraseId,error] = BA_InverseDepth_2viewBLS_Robust(FeatureBag, AbsolutePoses, PoseGraphMatrix, featureExtracted, usedPoses, CameraParams,sigma)
...
for featk = featureAvailableList %0:size(PoseGraphMatrix,1)-1
   ...
   for l = 0:length(poses_see_featk)-1
      ...
      if usedPoses(poses_see_featk(l+1)) == 2
         Jpose(2*meascount+1:2*meascount+2, 6*(poses_see_featk(l+1)-1)+1:6*(poses_see_featk(l+1)-1)+5) = Jr*[Jp Jq];
      else
          if usedPoses(poses_see_featk(l+1)) ~= 1      
              Jpose(2*meascount+1:2*meascount+2, 6*(poses_see_featk(l+1)-2)+5+1:6*(poses_see_featk(l+1)-2)+5+6) = Jr*[Jp Jq];
          end
      end
      ...
      if ref_idx_usePoses ~= 1      
         if usedPoses(ref_idx_usePoses) == 2
            Jpose(2*meascount+1:2*meascount+2, 6*(ref_idx_usePoses-1)+1:6*(ref_idx_usePoses-1)+5) = Jr*[Jpr Jqr];
         else
             if usedPoses(ref_idx_usePoses) ~= 1      
                Jpose(2*meascount+1:2*meascount+2, 6*(ref_idx_usePoses-2)+5+1:6*(ref_idx_usePoses-2)+5+6) = Jr*[Jpr Jqr];
             end
         end
      end
      
      residual(2*meascount+1:2*meascount+2) = residual_r;
      Jfeat(2*meascount+1:2*meascount+2, 3*featcount+1:3*featcount+3) = Jr*[Jtheta Jphi Jro];              
