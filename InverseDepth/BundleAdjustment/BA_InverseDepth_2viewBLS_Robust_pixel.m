function [FeatureBag, AbsolutePoses, CameraParams,PoseGraphMatrix,normHist, gradnormHist,EraseId,error] = BA_InverseDepth_2viewBLS_Robust_pixel(FeatureBag, AbsolutePoses, PoseGraphMatrix, featureExtracted, usedPoses, CameraParams,sigma)
...
while iter < maxIter && norm_iter > 1 && grad_norm > 1e-2
...
for featk = featureAvailableList %0:size(PoseGraphMatrix,1)-1
   ...
   for l = 0:length(poses_see_featk)-1
      ...
      [Jfc,Jcc,residual_tmp,flag] = dprojection_dfc_dcc(Cr_p_f, Ck_R_W, W_R_Cr,W_p_Cr, W_p_Ck, Ck_z_f,CameraParams,CameraModel,thresh);
...
% %         sigma = sigma * 1.05;
% % %         sigma = thresh*2;
%     elseif (norm_iter < normHist(iter-1)) 
%         thresh = thresh * 1.05;
%         sigma = sigma * 1.05;
% %         sigma = thresh*2;
%     end
