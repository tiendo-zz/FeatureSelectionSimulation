function VisualizeAllPosesReprojection(PoseGraphMatrix, AbsolutePoses, FeaturesBag, featureExtracted, usedPoses, newPose, usedPosesId,I_rgb, CameraParams)

global CameraModel

allposes = [usedPoses, newPose];
allposesId = usedPosesId;
I_full = [];
for k = allposes
...
for k = allposes
    ...
    for featIdx = visiblefeaturesIdx'
        ...
        Ck_P = Ck_T_Cv*[FeaturesBag(1:3,featIdx);1];
          ...
          Ck_reproj(countFeature,:)  = StaticDistort(CameraParams.fc,CameraParams.cc,CameraParams.kc,Ck_P(1:2)/Ck_P(3),CameraModel)';
%         Ck_reproj(countFeature,:) = CameraParams.K(1:2,:)*Ck_P;
