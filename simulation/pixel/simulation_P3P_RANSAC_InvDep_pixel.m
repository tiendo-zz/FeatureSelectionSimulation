function     [Rbest, tbest, bestInlier, bestGeneratorSet, featureUsed, measurementUsed,idx_3d] = simulation_P3P_RANSAC_InvDep_pixel(PoseGraphMatrix, ...
                                                                           AbsolutePoses,    ...
                                                                           featureExtracted, ...
                                                                           usedPoses, newPose,         ...
...
while iter < MaxIter
    ...
    % Compute pose
    select_point = w(picked_points_idx(1:3), :)';
    [C,p] = P3P_Solver(P(1:3,picked_points_idx(1:3)), CameraParams.Kinv*[select_point;ones(1,3)]);
    if isempty(C) || isempty(p)
        continue;
    end
    [R,t,~]=simulation_TruePose_P3P_pixel(C,p,P(1:3,picked_points_idx(4)),w(picked_points_idx(4), :)',CameraParams);
    
    % Compute inlier based on reprojection
    inlierIdx = simulation_ComputeInlier3D2DReprojection_pixel(P, w, CameraParams, R, t, threshold);
