function [FeatureBag, AbsolutePoses, CameraParams,normHist, gradnormHist,EraseId,error] = BA_InverseDepth_2viewBLS_pixel_simulation(FeatureBag, AbsolutePoses, PoseGraphMatrix, featureExtracted, usedPoses, CameraParams)
...
%       Ck_z_f_hat = m_theta_phi(1:2) / m_theta_phi(3);
      ...
      [Jfc,Jcc,residual_tmp,~] = dprojection_dfc_dcc_simulation(Cr_p_f, Ck_R_W, W_R_Cr,W_p_Cr, W_p_Ck, Ck_z_f,CameraParams);
%       
...
CameraParams.fc(1) = CameraParams.fc(1) + p_tilde(end-3);
CameraParams.fc(2) = CameraParams.fc(2) + p_tilde(end-2);
CameraParams.cc(1) = CameraParams.cc(1) + p_tilde(end-1);
CameraParams.cc(2) = CameraParams.cc(2) + p_tilde(end);
