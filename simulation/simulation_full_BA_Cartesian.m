function [AbsolutePoses,FeaturesBag,measurePoses] = simulation_full_BA_Cartesian(PoseGraphMatrix,AbsolutePoses,featureExtracted,NumOfPoses)
...
for newPose = 1:NumOfPoses
    if newPose==1
       ...
       if newPose==2
         ...
         % Triangulation
         FeaturesBag = LinearTriangulation_5pt(PairWiseMatches,PoseGraphMatrix, CameraParams, C2_R_C1', -C2_R_C1'*C2_t_C1);
         refPose = 1;
         scalePose = 2;
         else
           % change pixel value to homo 
           [Cn_R_Cr, Cn_t_Cr, InlierIdx, ~, Cr_P, Cn_z,idx_3d] = P3P_RANSAC2(PoseGraphMatrix, AbsolutePoses,  ...
                                                         featureExtracted,                      ...
...
%           end
          ...
          %% 6. nonlinear PnP
          [Cn_R_Cr, Cn_t_Cr, ~] = PnP_NL(Cn_R_Cr, Cn_t_Cr, Cr_P(:, InlierIdx), Cn_z(InlierIdx,:)',CameraParams);                  
      ...
      measurePoses = [measurePoses newPose];
       ...
       if length(measurePoses) > 2
           [FeaturesBag, AbsolutePoses, ~, ~,~] = BA_Cartesian_2viewBLS_Robust(FeaturesBag, AbsolutePoses, PoseGraphMatrix, featureExtracted, measurePoses, CameraParams, 1e-2);
%           [FeaturesBag, AbsolutePoses, ~, ~,~] = BA_Cartesian_2viewBLS_simulation(FeaturesBag, AbsolutePoses, PoseGraphMatrix, featureExtracted, measurePoses, CameraParams);
%           [FeaturesBag, AbsolutePoses, ~, ~,~] = BA_Cartesian_simulation(FeaturesBag, AbsolutePoses, PoseGraphMatrix, featureExtracted, measurePoses, CameraParams);
       end       
    end
